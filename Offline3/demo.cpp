...
void loadData()
{
	...
	for (int i=0;i<numSpotLights;i++)
	{
		...
		fin >> lightPosition.x >> lightPosition.y >> lightPosition.z;
		fin >> lightColor.r >> lightColor.g >> lightColor.b;
		PointLight currentLight(lightPosition, lightColor);

		fin >> lightDirection.x >> lightDirection.y >> lightDirection.z;
		fin >> cutoff;
		SpotLight currentSpotLight(currentLight, lightDirection, cutoff);

		spotLights.push_back(currentSpotLight);
	}
}
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("1705053_Offline3");
